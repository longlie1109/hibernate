...
public class XuatHoaDonGUI extends JDialog {
    ...
    private void xuLyHienThiHoaDon() throws InstantiationException, IllegalAccessException, UnsupportedLookAndFeelException {
        ...
        String hd = "<style> "
                ...
                + "}"
                + "</style>";
        hd += "<h1 style='text-align:center;'>HO�? �?ƠN THANH TO�?N</h1>";
        ...
        if (timMaUI.maGiamTimDuoc != null) {
            ...
            if (tongTien >= timMaUI.maGiamTimDuoc.getDieuKien()) {
                ...
                new MyDialog("Không đủ đi�?u kiện nhận ưu đãi!", MyDialog.ERROR_DIALOG);
    ...
    private void btnThanhToanActionPerformed(java.awt.event.ActionEvent evt) throws InstantiationException, IllegalAccessException, UnsupportedLookAndFeelException {
        ...
        if (txtTenKhach.getText().equals("")) {
            new MyDialog("Xin ch�?n khách hàng", MyDialog.ERROR_DIALOG);
            return;
        }
        if (txtMaGiam.getText().equals("")) {
            new MyDialog("Xin ch�?n mã giảm", MyDialog.ERROR_DIALOG);
            return;
        }
        xuLyHienThiHoaDon();
        btnInHoaDon.setEnabled(true);

        hoadonBUS.luuHoaDon(DlgTimKhach.khachHangTimDuoc.getMaKH(), nhanVien, tongTien, "�?ã thanh toán");
