...
public class PhieuNhapBUS {
    ...
    public ArrayList<PhieuNhap> getListPhieuNhapTheoGia(String giaThap, String giaCao) throws InstantiationException, IllegalAccessException, UnsupportedLookAndFeelException {
        try {
            int min = Integer.parseInt(giaThap);
            int max = Integer.parseInt(giaCao);
            if (max < min) {
                new MyDialog("HÃ£y nháº­p khoáº£ng giÃ¡ phÃ¹ há»£p!", MyDialog.ERROR_DIALOG);
                return null;
            }
            ...
            new MyDialog("HÃ£y nháº­p sá»‘ nguyÃªn cho khoáº£ng giÃ¡!", MyDialog.ERROR_DIALOG);
        }
        return null;
    }

    public ArrayList<PhieuNhap> getListPhieuNhapTheoNgay(String tuNgay, String denNgay) throws InstantiationException, IllegalAccessException, UnsupportedLookAndFeelException {
        try {
            ...
            if (max.before(min)) {
                new MyDialog("HÃ£y nháº­p khoáº£ng ngÃ y phÃ¹ há»£p!", MyDialog.ERROR_DIALOG);
                return null;
            }
            ArrayList<PhieuNhap> result = new ArrayList<PhieuNhap>();
            for (PhieuNhap pn : listPhieuNhap) {
            ...
            new MyDialog("HÃ£y nháº­p ngÃ y há»£p lá»‡ (dd/MM/yyy)!", MyDialog.ERROR_DIALOG);
        }
        return null;
    }
